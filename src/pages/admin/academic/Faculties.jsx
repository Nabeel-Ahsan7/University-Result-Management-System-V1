...
export default function FacultyManagement() {
    ...
    const handleEditFaculty = async () => {
        try {
            ...
            const response = await axios.put(
                `${API_URL}/faculty/${currentFaculty._id}`,
                formData,
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
            ...
            setFaculties(faculties.map(faculty =>
                faculty._id === currentFaculty._id ? response.data.faculty : faculty
            ));
    ...
    const handleDeleteFaculty = async () => {
        try {
            ...
            await axios.delete(`${API_URL}/faculty/${currentFaculty._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });

            setFaculties(faculties.filter(faculty => faculty._id !== currentFaculty._id));
    ...
    if (loading) return <div className="text-center py-12">Loading faculties...</div>;
    ...
    return (
        <div>
                        ...
                        {filteredFaculties.length === 0 ? (
                            <tr>
                                <td colSpan="2" className="px-6 py-4 text-center text-gray-500">
                                    {searchQuery ? 'No faculties match your search' : 'No faculties found'}
                                </td>
                            </tr>
                        ) : (
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                <DialogContent className="bg-white border-0 shadow-lg rounded-lg">
                    <DialogHeader className="border-b pb-3">
                        <DialogTitle className="text-xl font-semibold text-gray-800">Add New Faculty</DialogTitle>
                    ...
                    <div className="space-y-4 py-5">
                        <div className="space-y-2">
                            <Label htmlFor="name" className="text-gray-700 font-medium">Faculty Name</Label>
                            <Input
                                id="name"
                                name="name"
                                value={formData.name}
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                <DialogContent className="bg-white border-0 shadow-lg rounded-lg">
                    <DialogHeader className="border-b pb-3">
                        <DialogTitle className="text-xl font-semibold text-gray-800">Edit Faculty</DialogTitle>
                        <DialogDescription className="text-gray-600">
                    ...
                    <div className="space-y-4 py-5">
                        <div className="space-y-2">
                            <Label htmlFor="edit-name" className="text-gray-700 font-medium">Faculty Name</Label>
                            <Input
                                id="edit-name"
                                name="name"
                                value={formData.name}
            ...
            <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <DialogContent className="bg-white border-0 shadow-lg rounded-lg">
                    <DialogHeader className="border-b pb-3">
                        <DialogTitle className="text-xl font-semibold text-gray-800">Delete Faculty</DialogTitle>
                        <DialogDescription className="text-gray-600">
                    ...
                    <div className="py-5">
                        <p className="text-sm text-gray-700">
                            Faculty: <span className="font-medium text-[#025c53]">{currentFaculty?.name}</span>
                        </p>
                    </div>
                    <DialogFooter className="border-t pt-3 mt-2">
                        <Button
