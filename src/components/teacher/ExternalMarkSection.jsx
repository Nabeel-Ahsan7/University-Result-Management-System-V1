...
const ExternalMarkSection = () => {
    ...
    // Fetch assigned exams where teacher is an examiner (first, second, or third)
    useEffect(() => {
        const fetchAssignedExams = async () => {
            try {
                setLoading(true);
                const response = await teacherService.getAssignedExams('external');
    ...
    const handleSelectCourse = async (courseAssignmentId) => {
        ...
        const examinerId = selectedAssignment.courseAssignment.first_examiner_id;
        ...
        const formattedMarks = selectedAssignment.exams.map(exam => {
            ...
            return {
                ...
                studentType: exam.student_type // Add student type to identify improvement exams
            };
        });
    ...
    const determineExaminerType = (courseAssignment) => {
        // Get teacher ID from the JWT token
        ...
        if (token) {
            try {
                ...
                teacherId = payload._id || payload.id;
            } catch (e) {
                console.error('Error parsing token:', e);
            }
        }

        // Compare with course assignment examiner IDs

        if (teacherId === courseAssignment.first_examiner_id) {
            return 'first';
        } else if (teacherId === courseAssignment.second_examiner_id) {
            return 'second';
        } else if (teacherId === courseAssignment.third_examiner_id) {
            return 'third';
        }

        // If no match is found, check for exact string match (in case of type differences)
        if (String(teacherId) === String(courseAssignment.first_examiner_id)) {
            return 'first';
        } else if (String(teacherId) === String(courseAssignment.second_examiner_id)) {
            return 'second';
        } else if (String(teacherId) === String(courseAssignment.third_examiner_id)) {
            return 'third';
        }
    ...
    const handleSubmitMarks = async () => {
        ...
        try {
            ...
            // Get current teacher's examiner type for this course
            const examinerType = determineExaminerType(selectedAssignment.courseAssignment);
            ...
            for (const student of studentMarks) {
                ...
                // Use the confirmed examiner type rather than student.examinerType

                await teacherService.submitExternalMarks(
                    student.examId,
                    {
                        mark: Number(student.mark),
                        examinerType: examinerType // Use the confirmed examiner type
                    }
                );
            }
    ...
    return (
        <div className="p-6">
            ...
            {/* Remove the separate form at the bottom of the page */}
        </div>
    );
};
