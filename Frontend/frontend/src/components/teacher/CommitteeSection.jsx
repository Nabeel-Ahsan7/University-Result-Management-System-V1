...
const CommitteeSection = () => {
    ...
    const [approvingStatus, setApprovingStatus] = useState({ type: null, loading: false });
    ...
    useEffect(() => {
        const fetchCommittees = async () => {
            try {
                setLoading(true);
                const response = await teacherService.getCommittees();
    ...
    useEffect(() => {
        ...
        const fetchCommitteeMarks = async () => {
            try {
                setLoading(true);
                const response = await teacherService.getCommitteeMarks(selectedCommittee._id);
    ...
    // Add this logic to collect unique students from all courses when semester is selected
    useEffect(() => {
    ...
    const handleApproveMarks = async (type) => {
        if (!selectedCommittee || !selectedSemester) return;

        try {
            setApprovingStatus({ type, loading: true });
            setError('');
            setSuccessMessage('');

            const response = await teacherService.approveMarks({
            ...
            setSuccessMessage(`${type.charAt(0).toUpperCase() + type.slice(1)} marks approved successfully!`);

            // Refresh committee marks to update approval status
            const marksResponse = await teacherService.getCommitteeMarks(selectedCommittee._id);
            setCommitteeMarks(marksResponse.data);

            // Update approval statuses
            if (marksResponse.data.approvalStatuses) {
                const statusMap = {};
                marksResponse.data.approvalStatuses.forEach(status => {
                    if (status) {
                        statusMap[status.semester_id] = status;
                    }
                });
            ...
            console.error(`Error approving ${type} marks:`, err);
            setError(`Failed to approve ${type} marks. ${err.response?.data?.message || 'Please try again.'}`);
        } finally {
            setApprovingStatus({ type: null, loading: false });
        }
    };
    ...
    // Add this function to your CommitteeSection component

    // Modified function beginning
    const generateSemesterResultPDF = (committeeData, semesterData, results) => {
        try {
            ...
            const selectedSemesterName = semesters.find(sem => sem.id === selectedSemester)?.name || 'Unknown Semester';
            ...
            // Committee details
            doc.setFontSize(12);
            doc.text(`Committee: ${committeeData.name}`, 14, 30);
            doc.text(`Department: ${committeeData.department}`, 14, 38);
            doc.text(`Session: ${committeeData.session}`, 14, 46);
            ...
            results.forEach(result => {
                ...
                if (result.courseAssignment.semester._id === selectedSemester) {
                            ...
                            {committees.map(committee => (
                                ...
                                >
                                    ...
                                    <div className="mt-1 flex items-center">
                                        <span
                                            className={`px-2 py-1 text-xs rounded-full ${committee.isPresident
                        ...
                        <div className="bg-white p-4 rounded-lg shadow border border-gray-200">
                            ...
                            {!committeeMarks ? (
                                ...
                                <>
                                    ...
                                    {/* Add this right after the semester selection dropdown, before the approval actions section */}
                                    {selectedSemester && (
                                        <div className="mb-6">
                                    ...
                                    {/* Approval Actions - Only for President */}
                                    {selectedCommittee.isPresident && selectedSemester && (
                                        <div className="mb-6 p-4 bg-gray-50 rounded-md">
                                            <h4 className="text-md font-medium mb-3">Approval Actions</h4>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <button
                                                        onClick={() => handleApproveMarks('internal')}
                                                        disabled={approvingStatus.loading || approvalStatus[selectedSemester]?.internal_mark_status === 'approved'}
                                                        className={`w-full py-2 px-4 rounded-md text-white ${approvingStatus.loading && approvingStatus.type === 'internal'
                                                            ? '!bg-green-300'
                                                            : approvalStatus[selectedSemester]?.internal_mark_status === 'approved'
                                                                ? '!bg-gray-300 cursor-not-allowed'
                                                    ...
                                                    >
                                                        {approvingStatus.loading && approvingStatus.type === 'internal' ? (
                                                ...
                                                <div>
                                                    ...
                                                    <button
                                                        onClick={() => handleApproveMarks('external')}
                                                        disabled={approvingStatus.loading || approvalStatus[selectedSemester]?.external_mark_status === 'approved'}
                                                        className={`w-full py-2 px-4 rounded-md text-white ${approvingStatus.loading && approvingStatus.type === 'external'
                                                            ? '!bg-green-300'
                                                            : approvalStatus[selectedSemester]?.external_mark_status === 'approved'
                                                                ? '!bg-gray-300 cursor-not-allowed'
                                                                : '!bg-green-600 hover:bg-green-700'
                                                    ...
                                                    >
                                                        {approvingStatus.loading && approvingStatus.type === 'external' ? (
                                                            <span className="flex items-center justify-center">
                                                                <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
